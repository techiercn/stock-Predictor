...
def fetch_macro_data(api_key):
    """Fetches Fed rates with a 2026 fallback."""
    try:
        ...
        return 3.75, 0.0 # April 2026 Baseline

def get_recommendation(score):
    """Logic for color-coded signals."""
...
def get_av_analysis(symbol, api_key, rate_delta, cur_rate):
    """Robust API call with connection error handling."""
    API_URL = "https://alphavantage.co"
    params = {"function": "NEWS_SENTIMENT", "tickers": symbol, "apikey": api_key}
    
    try:
        # Added timeout=10 to prevent ConnectionErrors from hanging
        ...
        return {
            ...
            "Fed_Rate": f"{cur_rate}%", "Market_Context": "April 2026 Resilience"
        }
...
if not FRED_KEY or not AV_KEY:
    ...
    if scan_btn:
        ...
        # Save to session state so it survives the Export rerun
        st.session_state.results_data = temp_results

# --- Display Results and Export (Always renders if data exists) ---
if st.session_state.results_data:
    ...
    for item in st.session_state.results_data:
        with st.expander(f"{item['Ticker']} - Recommendation: {item['Recommendation']}"):
            ...
            with c2:
                st.write(f"**Geopolitical Context**: Tracking 2026 energy shocks and Strait of Hormuz volatility.")
                st.write(f"**Fed Interest Rates**: Steady at {item['Fed_Rate']} (Trend: {'Tightening' if rate_delta > 0 else 'Stable/Easing'}).")
                st.write(f"**Market Performance**: {item['Market_Context']}")
